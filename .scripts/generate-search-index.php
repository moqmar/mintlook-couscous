 ...
 * MIT License, which accompanies this distribution and is available at
...
function directory($dir, &$results = array()) {
    $files = scandir($dir);
    foreach ($files as $filename){
        if (!is_dir($dir . DIRECTORY_SEPARATOR . $filename) && strpos($filename, ".") !== 0) {
            $results[] = $dir . "/" . $filename;
        } else if (strpos($filename, ".") !== 0) {
            directory($dir . DIRECTORY_SEPARATOR . $filename, $results);
        }
    }
    return $results;
}
...
foreach ($files as $filename) {
    ...
    if (strpos($content, "<!-- THIS DOCUMENT IS INDEXED -->") !== false) {
		echo "Indexing $filename...\n";
		
		$cstart = strpos($content, "<!-- CONTENT START -->");
		$cend = strpos($content, "<!-- CONTENT END -->");
		if ($cstart !== false && $cend !== false) {
			$article = substr($content, $cstart, $cend - $cstart);
		} else {
			echo "File doesn't have a <!-- CONTENT START --> and/or <!-- CONTENT END -->";
			continue;
		}
		
		$link = preg_replace('#index\.html$#','',substr($filename, strlen($dir)));
		
		$title = "";
		try {
			$title = DOMDocument::loadHTML($article)->getElementsByTagName("h1")[0]->textContent;
		} catch (Exception $e) {}
		if (strlen($title) == 0) $title = $link;

		$description = "";
		$dstart = strpos($content, "<template search-description>");
		if ($dstart !== false) {
			$description = substr($content, $dstart + 29);
			$dend = strpos($description, "</template>");
			if ($dend !== false) $description = substr($description, 0, $dend);
		}

		$index[] = [ "url" => $link, "title" => $title, "content" => Html2Text::convert($article), "description" => $description ];
    }
}

$result = json_encode($index);
$indexfile = preg_replace('#/+#','/',"$dir/search-index.json");
file_put_contents($indexfile, $result);
echo "Created index file at $indexfile (" . strlen($result) . " Bytes)\n";
